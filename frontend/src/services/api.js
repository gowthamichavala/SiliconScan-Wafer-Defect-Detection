const BASE_URL = 'https://siliconscan-wafer-defect-detection.onrender.com'
...
export async function predictSingle(file) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      errorData.detail || `Prediction failed (${res.status})`
    )
  }
...
export async function predictBatch(fileList) {
  ...
  const res = await fetch(`${BASE_URL}/api/batch-predict`, {
    method: 'POST',
    body: formData
  })
  ...
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({}))
    throw new Error(
      errorData.detail || `Batch prediction failed (${res.status})`
    )
  }
...
export async function getHistory(params = {}) {
  ...
  if (params.class_filter) {
    query.append('class_filter', params.class_filter)
  }

  if (
    params.defective_filter !== undefined &&
    params.defective_filter !== null
  ) {
  ...
  const res = await fetch(
    `${BASE_URL}/api/history${queryString ? `?${queryString}` : ''}`
  )
